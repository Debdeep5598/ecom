...
const Navbar = () => {
  ...
  <div className="container mx-auto flex items-center justify-between">
    ...
    {/* Desktop Icons */}
    <div className="hidden md:flex items-center space-x-6 text-gray-900">  {/* ✅ Changed from text-white */}
      ...
      >
        ...
        {dropdownOpen && (
          <div 
            ref={dropdownRef}
            className="absolute right-0 mt-2 w-48 bg-white/10 backdrop-blur-md text-gray-900 shadow-lg rounded-md p-2 border border-white/20"
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
          >
            <ul className="py-2">
              {isLoggedIn ? (
                <>
                  <li
                    className="px-4 py-2 hover:bg-cyan-500/20 cursor-pointer transition"
                    onClick={() => {
                      setIsLoggedIn(false);
                      setDropdownOpen(false);
                    }}
                  ...
                  <Link href="/account" className="block px-4 py-2 hover:bg-cyan-500/20 transition">Your Account</Link>
                  <Link href="/orders" className="block px-4 py-2 hover:bg-cyan-500/20 transition">Your Orders</Link>
                  <Link href="/wishlist" className="block px-4 py-2 hover:bg-cyan-500/20 transition">Wish List</Link>
                </>
              ) : (
                <>
                  <Link href="/signin" className="block px-4 py-2 hover:bg-cyan-500/20 transition">Sign In</Link>
                  <Link href="/signin" className="block px-4 py-2 hover:bg-cyan-500/20 transition">Your Orders</Link>
                  <Link href="/signin" className="block px-4 py-2 hover:bg-cyan-500/20 transition">Wish List</Link>
                </>
              )}
            </ul>
          </div>
